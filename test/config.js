...
describe('config', function () {
  it('reads file', function () {
    ...
    expect(conf.email.to).to.deep.equal({
      0: 'email.to.1',
      1: 'email.to.2'
    })

    expect(conf).to.have.property('apps')
    expect(conf.apps).to.deep.equal({
      'app.id': {
        name: 'app.name'
      }
    })
  ...
  it('returns correct defaults', function () {
    // ## Setup
    fs.writeFileSync(testConfigFilePath, JSON.stringify({}))
    // ## Test
    config.use(testConfigFilePath)
    let conf = config.getConfig()
    // ## Assert
    ...
    expect(conf).to.have.property('checkInterval')
    expect(conf.checkInterval).to.be.instanceOf(duration)
    expect(conf.checkInterval.toString()).to.equal('1h')

    expect(conf).to.have.property('smtp')
    expect(conf.smtp).to.have.property('ssl')
    expect(conf.smtp.ssl).to.equal(true)

    expect(conf).to.have.property('email')
    expect(conf.email).to.have.property('from')
    expect(conf.email.from).to.equal('AppStore Monitor')
